...
class DatabaseConnection:
    ...
    def fetch_data(self, schema_name, table_name, conditions_dict):
        ...
        query = "SELECT * FROM {schema}.{table} {where_str}" "".format(
            schema=schema_name, table=table_name, where_str=where_str
        )
    ...
    def insert(self, data):
        ...
        if data.get("Model") and data.get("Data") is not None:
            ...
            for link in data.get("Links", []):
                foreign_key_models = (
                    self.session.query(link["OriginModel"])
                    .filter_by(**dict(link["OriginSelection"]))
                    .all()
                )
                if foreign_key_models and len(foreign_key_models) == 1:
                    data["Data"][link["SecondaryKey"]] = getattr(
                        foreign_key_models[0], link["OriginPrimaryKey"]
                    )
                else:
            ...
            with open(data.get("DataFile"), "r") as f:
                ...
                for row in reader:
                    ...
                    for link in data.get("Links", []):
                        ...
                        foreign_objects = (
                            self.session.query(link["OriginModel"])
                            .filter_by(**selection_filter)
                            .all()
                        )
                        if foreign_objects and len(foreign_objects) == 1:
                            row[link["SecondaryKey"]] = getattr(
                                foreign_objects[0], link["OriginPrimaryKey"]
                            )
                        else:
                            raise Exception("Error obtaining foreign key data.")
                    ...
                    n_rows += (
                        1
                        if self.insert({"Model": data["Model"], "Data": data_entry})
                        != -1
                        else 0
                    )
            if self.echo:
...
class DBConnMaker:
    def __init__(self, protocol, host, user, password, database, echo):
        self.db_conn = DatabaseConnection(
            protocol, host, user, password, database, echo
        )
