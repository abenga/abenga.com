...
class LoginSession(Base):
    ...
    person_id = Column(Integer, ForeignKey("people.id"), name='person_id', quote=False)
    session_id = Column(String(128), unique=True, nullable=False, name='session_id', quote=False)
    time_started = Column(DateTime, nullable=False, name='time_started', quote=False)
    last_action_time = Column(DateTime, nullable=False, name='last_action_time', quote=False)
...
class Person(Base):
    ...
    username = Column(String, unique=True, nullable=False, name='username', quote=False)
    email = Column(String, unique=True, nullable=False, name='email', quote=False)
    primary_phone_number = Column(String(32), nullable=False, name='primary_phone_number', quote=False)
    login_type = Column(String(255), nullable=False, name='login_type', quote=False)
    ...
    contact_email = Column(String, unique=True, name='contact_email', quote=False)
    other_phone_numbers = Column(JSONB, unique=True, name='other_phone_numbers', quote=False)
    postal_address = Column(JSONB, unique=True, name='postal_address', quote=False)
    physical_address = Column(JSONB, unique=True, name='physical_address', quote=False)
