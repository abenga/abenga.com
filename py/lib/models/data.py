...
class Author(Base):
    ...
    id = Column(
        Integer, ForeignKey("core.people.id"), primary_key=True, name="id", quote=False
    )
...
class PostSeries(Base):
    ...
    author_id = Column(
        Integer, ForeignKey("data.authors.id"), name="author_id", quote=False
    )
    title = Column(String, nullable=False, name="title", quote=False)
    joined_title = Column(
        String, unique=True, nullable=False, name="joined_title", quote=False
    )
...
class Post(Base):
    ...
    series_id = Column(
        Integer,
        ForeignKey("data.post_series.id"),
        nullable=True,
        name="series_id",
        quote=False,
    )
    ...
    references_html = Column(
        String, nullable=False, name="references_html", quote=False
    )
    cover_image_path = Column(String, name="cover_image_path", quote=False)
    tags = Column(ARRAY(String), name="tags", quote=False)
